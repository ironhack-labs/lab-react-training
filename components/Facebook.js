...
function Facebook(){
    const [coloredCopuntry, setColoredCountry] = useState('');
    ...
    return(
        <div>
           ...
           {facebookData.map((person, index) =>{
               return (
                   <div style={{ backgroundColor: `${person.country === coloredCopuntry ? 'green' : 'white'}`}} className="facebook-basic-container" key={index}>
