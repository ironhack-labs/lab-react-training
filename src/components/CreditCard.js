...
class CreditCard extends React.Component {
  render() {
    ...
    const renderImg = ()=>{
        ...
        return <LogoImg src={process.env.PUBLIC_URL+ `/img/master-card.svg`} alt="Mastercard"/>
        }
    }
    ...
    return (
      <CreditCardDiv>
        ...
        <CardP>Expires <span>{expirationMonth}/{expirationYear.toString().slice(2)}</span> <span>{bank}</span></CardP>
        <CardP>{owner}</CardP>
      </CreditCardDiv>
    );
  }
