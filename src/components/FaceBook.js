...
export class FaceBook extends Component {
  constructor(props) {
    ...
    this.countriesArray = [
      ...new Set([...this.profiles].map((profile) => profile.country)),
    ];
    this.componentStyle = (country) =>
  ...
  getCountryButtons(arrOfCountries) {
    const countryButtons = [...arrOfCountries].map((country, index) => {
      return (
        <button
          key={index}
          style={this.componentStyle(country)}
          onClick={() => this.updateSelectedCountry({ country })}
        >
          {country}
        </button>
      );
